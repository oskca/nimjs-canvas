...
proc newCanvas(id: cstring, w, h: int = 200): Canvas =
  let canvas = dom.document.createElement("canvas").Canvas
  ...
  canvas.style.margin = "2em"
  return canvas

proc canvasTest()=
  let
    canvas = newCanvas("info-canvas", 25, 1)
...
proc dataTest()=
  let
    canvas = newCanvas("data-canvas", 6, 3)
...
proc textTest()=
  ...
  ctx.fillStyle = "black"
  ctx.textBaseline = TextBaseLineHanging
  ...
  ctx.fillStyle = "green"
  ctx.strokeStyle = "blue"
  ctx.textAlign = TextAlignRight
  ctx.textBaseline = TextBaseLineBottom
...
proc lineTest()=
  ...
  ctx.lineCap = LineCapSquare
  ctx.lineJoin = LineJoinMiter
  ctx.lineWidth = 5
  ctx.beginPath()
  ctx.moveTo(4, 150)
...
proc gradAndPattTest()=
  let
    canvas = newCanvas("grad-and-patt-canvas")
    ctx = canvas.getContext2D()
...
proc shadowTest()=
  let
    canvas = newCanvas("shadow-canvas")
    ctx = canvas.getContext2D()
...
proc paths1Test()=
  let
    canvas = newCanvas("paths-1-canvas")
    ctx = canvas.getContext2D()
...
proc paths2Test()=
  let
    canvas = newCanvas("paths-2-canvas")
    ctx = canvas.getContext2D()
  ...
  echo ctx.isPointInPath(20, 180, FillRuleNonZero)


proc transformsTest()=
  let
    canvas = newCanvas("transforms-canvas")
    ctx = canvas.getContext2D()
...
proc compositingTest()=
  let
    canvas = newCanvas("compositing-canvas")
    ctx = canvas.getContext2D()
  ...
  ctx.globalAlpha = 0.8
  ctx.globalCompositeOperation = CompositeOptSourceAtop
  ctx.fillStyle = rgb(250, 10, 65)

  ctx.arc(140, 140, 100, 0, 2 * Pi)
  ctx.fill()

  ctx.lineJoin = LineJoinBevel
...
proc imageDraw1Test()=
  let
    canvas = newCanvas("image-drawing-1-canvas")
    ctx = canvas.getContext2D()
...
proc imageDraw2Test()=
  let
    canvas = newCanvas("image-drawing-2-canvas")
    ctx = canvas.getContext2D()
...
proc dynamicCreationTest()=
  ...
  var canvas = newCanvas("dynamicsTest")
  let ctx = canvas.getContext2D()

  ctx.save()
  ctx.textAlign = TextAlignCenter
  ctx.textBaseline = TextBaseLineMiddle
  ctx.strokeStyle=rgb(colRed)
  ctx.strokeText("I'm new here.", 50.0, 50, 100.2)
  ctx.restore()

# The dom on load
dom.window.onload = proc(e: dom.Event) =
  dom.window.document.fgColor = "black"
  # b.style.color = "black"
  canvasTest()
  dataTest()
  rectTest()
